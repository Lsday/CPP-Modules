...
/*   Updated: 2021/12/15 14:51:23 by oronda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Karen::setLevelEnum(std::string level)
{
	for (size_t i = 0; i < 4; i++)
	{
		if (!level.compare(levels[i]))
		{
			lvl = (Karen::logLevel)i;
			break;
		}
	}
}
...
void Karen::complain(std::string level)
{
	...
	switch(lvl)
	{
		case (Karen::logLevel::DEBUG) :
			std::cout << "[ DEBUG ]"<< std::endl;
			debug();

		case (Karen::logLevel::INFO) :
			std::cout << "[ INFO ]"<< std::endl;
			info();

		case (Karen::logLevel::WARNING) :
			std::cout << "[ WARNING ]"<< std::endl;
			warning();
		
		case (Karen::logLevel::ERROR) :
